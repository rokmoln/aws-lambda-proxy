...
let loadLambdas = async function ({ app, lambdas }) {
  ...
  Promise.all(_lodashFirecloud.default.map(lambdaLocations, async function ({ ctx, lambda, location }) {
    ...
    await (async createError => {try {return await _env.default.hooks.preRouteSetup({ env: _env.default, route });} catch (_awaitTraceErr) {let err = createError();_awaitTraceErr.stack += "\n...\n" + err.stack;throw _awaitTraceErr;}})(() => new Error());
...
let middleware = function (route) {
  ...
  return async function (req, res, _next) {
    ...
    await (async createError => {try {return await _env.default.hooks.preHandle({ e, env: _env.default, route, req, res });} catch (_awaitTraceErr2) {let err = createError();_awaitTraceErr2.stack += "\n...\n" + err.stack;throw _awaitTraceErr2;}})(() => new Error());
